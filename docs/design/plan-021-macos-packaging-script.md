...
5. Create `untype.app` with `untype`, `untype-input-helper`, `Info.plist`, and optional icon; `untype` itself is the bundle executable and opens UI mode for no-argument app-bundle launches.
...
9. Write the conventional `Contents/PkgInfo` marker and avoid packaging local extended attributes as AppleDouble `._*` zip entries.
...
- The generated app contains `Info.plist`, `untype`, and `untype-input-helper`; `untype-launcher` was removed on 2026-05-27 to align Accessibility/Input Monitoring identity with the process installing the global hotkey event tap.
